...
exports.Browsers = {
    ...
    Fazone: browser => ['Fazone', browser, '3.0'],
...
function decryptWA(message, macKey, encKey, decoder, fromMe = false) {
    ...
    if (commaIndex < 0)
        throw new Constants_1.FazoneError('invalid message', { message }); // if there was no comma, then this message must be not be valid
    ...
    if (data.length > 0) {
        if (typeof data === 'string') {
            ...
            if (!macKey || !encKey) {
                throw new Constants_1.FazoneError('recieved encrypted buffer when auth creds unavailable', { message });
            }
            /*
            ...
            */
            ...
            if (checksum.equals(computedChecksum)) {
                ...
                throw new Constants_1.FazoneError('checksum failed', {
...
async function decodeMediaMessageBuffer(message, fetchRequest) {
    ...
    */
    ...
    if (!type) {
        throw new Constants_1.FazoneError('unknown message type', message);
    }
    if (type === Constants_1.MessageType.text || type === Constants_1.MessageType.extendedText) {
        throw new Constants_1.FazoneError('cannot decode text message', message);
    }
    ...
    if (message.productMessage) {
        ...
        if (!product)
            throw new Constants_1.FazoneError('product has no image', message);
    ...
    if (buffer.length <= 10) {
        throw new Constants_1.FazoneError('Empty buffer returned. File has possibly been deleted from WA servers. Run `client.updateMediaMessage()` to refresh the url', { status: 404 });
    }
    ...
    throw new Constants_1.FazoneError('Decryption failed, HMAC sign does not match', { status: 400 });
}

...
class WAConnection extends events_1.EventEmitter {
    constructor() {
        ...
        this.browserDescription = Utils.Browsers.Fazone('Chrome');
        ...
        this.logger = logger.child({ class: 'Fazone' });
        /** log messages */
    ...
    async query(q) {
        ...
        if (expect200 && response.status && Math.floor(+response.status / 100) !== 2) {
            ...
            throw new Constants_1.FazoneError(`Unexpected status in '${json[0] || 'generic query'}': ${http_1.STATUS_CODES[response.status]}(${response.status})`, { query: json, message, status: response.status });
        }
